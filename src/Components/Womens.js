...
import womensData from './files/womens.json'
import { Link } from 'react-router-dom'
import '../css/ItemsPage.css'
import b1 from "../assets/banner3.jpg";
import b2 from "../assets/banner4.webp";
...
const Womens = () => {
  ...
  const filterprod =
    selectProd.length === 0
      ? womensData
      : womensData.filter((fil) => selectProd.includes(fil.title));
    ...
    <div id='items'>
        ...
        <div className="d-flex">
          <div className="p-2 m-2">
            ...
            {Array.from(new Set(womensData.map((product) => product.title))).map(
              (title) => {
                return (
                  <div key={title}>
              ...
              {filterprod.map((item) => (
                <div className="col-md-4">
                  <div className="card">
                    ...
                    <Link to={`/womens/${item.id}`} className="card-title">
                      <img
                        src={item.image}
                        style={{ height: "290px" }}
