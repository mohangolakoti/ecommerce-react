...
import kidsData from "./files/kids.json";
import { Link } from "react-router-dom";
import '../css/ItemsPage.css'
import b1 from "../assets/banner7.jpg";
import b2 from "../assets/banner6.jpg";
...
const Kids = () => {
  ...
  const filterprod =
    selectProd.length === 0
      ? kidsData
      : kidsData.filter((fil) => selectProd.includes(fil.title));
      ...
      <section className="mens mx-auto my-5 d-xxl py-1 ">
        <div className="d-flex">
          <div className="p-2 m-2">
            ...
            {Array.from(new Set(kidsData.map((product) => product.title))).map(
              (title) => {
                return (
                  <div key={title}>
              ...
              {filterprod.map((item) => (
                <div className="col-md-4">
                  <div className="card">
                    ...
                    <Link to={`/kids/${item.id}`} className="card-title">
                      <img
                        src={item.image}
                        style={{ height: "290px" }}
